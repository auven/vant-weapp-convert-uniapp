...
const packages = path.join(__dirname, '../src/packages')
const component = path.join(__dirname, '../components')

fs.copy(packages, component).then(() => {
	console.log('---- 同步完成 ----');
  const delFileLists = glob.sync(component + '/**/*.{json,md,bak}')
  delFileLists.push(...glob.sync(component + '/**/demo.vue'))
  delFileLists.push(...glob.sync(component + '/dashboard'))
	// 删除json  md 等不必要文件
